#!/usr/bin/env python3
"""
Plot User Spread Data
=====================

Plot the specific spread data file requested by user.
"""
...
def plot_user_spread():
    """Plot the user's spread data"""
    
    # Load the user's spread data
    ...
    if len(trades) > 0:
        ...
        ax1.plot(sequential_idx, trades['price'], linewidth=0.5, alpha=0.7, color='blue')
        ax1.set_title('DEBM1_25 vs DEBQ1_25 Spread - Trade Prices', fontsize=14, fontweight='bold')
    ...
    if len(orders) > 0:
        ...
        sequential_idx_orders = range(len(sample_orders))
        ax2.scatter(sequential_idx_orders, sample_orders['price'], s=1, alpha=0.5, color='green')
